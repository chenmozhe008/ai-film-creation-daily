...
```mermaid
flowchart TB
    subgraph Collection["📥 数据采集层 Collection"]
        direction LR
        R[RSS 18+ feeds] --> P[Processing]
        X[X / Twitter 100+ 账号] --> P
        H[HN / GitHub] --> P
        W[web_search] --> P
        WX[微信公众号 10+ 账号] --> P
        B[B站 bili-cli] --> P
    ...
    subgraph Processing["⚙️ 处理层 Processing"]
        direction LR
        T1[时间校验 24h/36h] --> T2[创作者关联度评分]
        T2 --> T3[事件级去重]
        T3 --> T4[来源权重排序]
        T4 --> T5[板块归类]
        T5 --> T6[摘要标准化]
